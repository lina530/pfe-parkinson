# ============================================
# app_cloud.py — Système de Détection Parkinson
# Design Médical Professionnel — NeuroGait
# ============================================
...
.proba-value    { font-family:'Playfair Display',serif; font-size:1.6rem; font-weight:700; color:#0A2342; }
...
with st.sidebar:
    ...
    st.markdown("<br><div style='font-size:0.72rem;color:rgba(255,255,255,0.3);text-align:center;line-height:1.6;'>PFE — Ingénierie Biomédicale<br>Pr. Youssef Ibnatta · 2026</div>", unsafe_allow_html=True)
...
if mode == "📁 Analyse Fichier":
    ...
    if uploaded:
        ...
        try:
            ...
            with k1:
                st.markdown(f'<div class="kpi-card info"><div class="kpi-label">⏱ Durée totale</div><div class="kpi-value">{duree:.0f}s</div><div class="kpi-sub">{len(df)} échantillons · 64 Hz</div></div>', unsafe_allow_html=True)
            with k2:
                st.markdown(f'<div class="kpi-card norm"><div class="kpi-label">✅ Marche normale</div><div class="kpi-value">{len(df[df["label"]==1])}</div><div class="kpi-sub">échantillons sains</div></div>', unsafe_allow_html=True)
            with k3:
                st.markdown(f'<div class="kpi-card fog"><div class="kpi-label">🔴 FoG annotés</div><div class="kpi-value">{fog_real}</div><div class="kpi-sub">fenêtres pathologiques</div></div>', unsafe_allow_html=True)
            with k4:
            ...
            with cg1:
                ...
                fig.add_trace(go.Scatter(y=df['ankle_z'].values[:3000], mode='lines',
                    line=dict(color='#1B4F72', width=1.2), name='Axe Z'))
                # Zones FoG
                in_fog = False; sf = 0
        ...
        <div style='background:white;border-radius:14px;padding:56px;
            ...
            <div style='color:#95A5A6;font-size:0.92rem;max-width:380px;margin:0 auto;line-height:1.6;'>
                Glisse un fichier <b>.txt</b> du dataset Daphnet FoG pour
                démarrer l'analyse complète de la marche et la détection du FoG.
            </div>
        </div>
        """, unsafe_allow_html=True)
...
# ============================================
    ...
    if st.session_state.get('running', False):
        ...
        while st.session_state.get('running', False):
            ...
            bar_color = "#E74C3C" if last_proba>60 else "#27AE60"
            proba_box.markdown(f"""
            <div class="proba-container">
                <div class="proba-label">Probabilité FoG</div>
                <div class="proba-bar-bg">
                    <div class="proba-bar-fill" style="width:{last_proba}%;background:{bar_color};"></div>
                </div>
                <div class="proba-value" style="color:{bar_color};">{last_proba}%</div>
            </div>""", unsafe_allow_html=True)

            stats_box.markdown(f"""
            <div style='background:white;border-radius:12px;padding:16px;box-shadow:0 2px 8px rgba(0,0,0,0.05);'>
                <div style='font-size:0.7rem;font-weight:600;color:#7F8C8D;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;'>Stats Session</div>
                <div style='display:flex;justify-content:space-between;margin-bottom:6px;'>
                    <span style='color:#27AE60;font-weight:600;'>🟢 Normal</span>
                    <span style='font-family:Playfair Display;font-weight:700;color:#0A2342;'>{n_normal}</span>
                </div>
                <div style='display:flex;justify-content:space-between;'>
                    <span style='color:#E74C3C;font-weight:600;'>🔴 FoG</span>
                    <span style='font-family:Playfair Display;font-weight:700;color:#E74C3C;'>{n_fog}</span>
                </div>
            </div>""", unsafe_allow_html=True)
